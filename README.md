...
| 1단계 | 한국어 데이터셋 생성 | 완료 |
| 2단계 | 전처리 파이프라인 구축 | 예정 |
| 3단계 | TF-IDF + LR 베이스라인 모델 | 예정 |
| 4단계 | BERT 모델 파인튜닝 | 예정 |
| 5단계 | FastAPI 서버 구축 및 배포 | 예정 |
...
| 고성능 모델 | KR-ELECTRA / multilingual-BERT (HuggingFace Transformers) |
...
spam-classifier/
├── data/
│   └── korean_spam_dataset.csv   # 생성 완료 (5,000개)
├── generate_dataset.py           # 데이터셋 생성 스크립트 (완료)
├── src/                          # 예정
│   ├── data/
│   │   ├── loader.py             # 데이터 로딩
...
```bash
py generate_dataset.py   # data/korean_spam_dataset.csv 재생성
```
